// V6.1.x — Rewrite Khoa học ky-data.
const fs = require('fs');
const path = 'engine/data/nhanxet-ky.json';
const KH = require('./data/khoahoc-v61.js');

let total = 0, ok = 0, bad = [];
for (const k of Object.keys(KH)) {
    for (const tier of Object.keys(KH[k])) {
        for (const s of KH[k][tier]) {
            total++;
            const L = s.length;
            if (L >= 90 && L <= 110) ok++;
...
for (const k of Object.keys(KH)) {
    for (const tier of Object.keys(KH[k])) {
        for (const s of KH[k][tier]) {
            // Khoa học chỉ lớp 4-5 — test gradeLevel=5 (chặt nhất)
...
const subj = ky.subjects.khoahoc;
if (!subj) { console.log('⛔ Không tìm thấy subjects.khoahoc'); process.exit(1); }
for (const k of Object.keys(KH)) {
    subj[k] = {};
    for (const tier of Object.keys(KH[k])) subj[k][tier] = KH[k][tier];
}
ky.meta.note_v61x_khoahoc = 'V6.1.x (2026-05-23): rewrite Khoa học ky-data — 3 kỳ × 4 tier, dạng học bạ TT27, band 90-110.';
fs.writeFileSync(path, JSON.stringify(ky, null, 2) + '\n', 'utf8');
console.log('\n✅ Đã patch ' + path + ' (khoahoc ky-data)');
