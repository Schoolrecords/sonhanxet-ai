// V6.1.x — Rewrite Mĩ thuật ky-data.
const fs = require('fs');
const path = 'engine/data/nhanxet-ky.json';
const MT = require('./data/mithuat-v61.js');

let total = 0, ok = 0, bad = [];
for (const k of Object.keys(MT)) {
    for (const tier of Object.keys(MT[k])) {
        for (const s of MT[k][tier]) {
            total++;
            const L = s.length;
            if (L >= 90 && L <= 110) ok++;
...
const subj = ky.subjects.mithuat;
if (!subj) { console.log('⛔ Không tìm thấy subjects.mithuat'); process.exit(1); }
for (const k of Object.keys(MT)) {
    subj[k] = {};
    for (const tier of Object.keys(MT[k])) subj[k][tier] = MT[k][tier];
}
ky.meta.note_v61x_mithuat = 'V6.1.x (2026-05-23): rewrite Mĩ thuật ky-data — 3 kỳ × 4 tier, dạng học bạ TT27, band 90-110.';
fs.writeFileSync(path, JSON.stringify(ky, null, 2) + '\n', 'utf8');
console.log('\n✅ Đã patch ' + path + ' (mithuat ky-data)');
