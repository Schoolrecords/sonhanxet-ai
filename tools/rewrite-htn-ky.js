// V6.1.x — Rewrite HĐTN ky-data.
const fs = require('fs');
const path = 'engine/data/nhanxet-ky.json';
const HT = require('./data/htn-v61.js');

let total = 0, ok = 0, bad = [];
for (const k of Object.keys(HT)) {
    for (const tier of Object.keys(HT[k])) {
        for (const s of HT[k][tier]) {
            total++;
            const L = s.length;
            if (L >= 90 && L <= 110) ok++;
...
const subj = ky.subjects.htn;
if (!subj) { console.log('⛔ Không tìm thấy subjects.htn'); process.exit(1); }
for (const k of Object.keys(HT)) {
    subj[k] = {};
    for (const tier of Object.keys(HT[k])) subj[k][tier] = HT[k][tier];
}
ky.meta.note_v61x_htn = 'V6.1.x (2026-05-23): rewrite HĐTN ky-data — 3 kỳ × 4 tier, dạng học bạ TT27, band 90-110.';
fs.writeFileSync(path, JSON.stringify(ky, null, 2) + '\n', 'utf8');
console.log('\n✅ Đã patch ' + path + ' (htn ky-data)');
