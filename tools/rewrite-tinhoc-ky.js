// V6.1.x — Rewrite Tin học ky-data.
const fs = require('fs');
const path = 'engine/data/nhanxet-ky.json';
const TH = require('./data/tinhoc-v61.js');

let total = 0, ok = 0, bad = [];
for (const k of Object.keys(TH)) {
    for (const tier of Object.keys(TH[k])) {
        for (const s of TH[k][tier]) {
            total++;
            const L = s.length;
            if (L >= 90 && L <= 110) ok++;
...
const subj = ky.subjects.tinhoc;
if (!subj) { console.log('⛔ Không tìm thấy subjects.tinhoc'); process.exit(1); }
for (const k of Object.keys(TH)) {
    subj[k] = {};
    for (const tier of Object.keys(TH[k])) subj[k][tier] = TH[k][tier];
}
ky.meta.note_v61x_tinhoc = 'V6.1.x (2026-05-23): rewrite Tin học ky-data — 3 kỳ × 4 tier, dạng học bạ TT27, band 90-110.';
fs.writeFileSync(path, JSON.stringify(ky, null, 2) + '\n', 'utf8');
console.log('\n✅ Đã patch ' + path + ' (tinhoc ky-data)');
