// V6.1.x — Rewrite Đạo đức ky-data.
const fs = require('fs');
const path = 'engine/data/nhanxet-ky.json';
const DD = require('./data/daoduc-v61.js');

let total = 0, ok = 0, bad = [];
for (const k of Object.keys(DD)) {
    for (const tier of Object.keys(DD[k])) {
        for (const s of DD[k][tier]) {
            total++;
            const L = s.length;
            if (L >= 90 && L <= 110) ok++;
...
const subj = ky.subjects.daoduc;
if (!subj) { console.log('⛔ Không tìm thấy subjects.daoduc'); process.exit(1); }
for (const k of Object.keys(DD)) {
    subj[k] = {};
    for (const tier of Object.keys(DD[k])) subj[k][tier] = DD[k][tier];
}
ky.meta.note_v61x_daoduc = 'V6.1.x (2026-05-23): rewrite Đạo đức ky-data — 3 kỳ × 4 tier, dạng học bạ TT27, band 90-110.';
fs.writeFileSync(path, JSON.stringify(ky, null, 2) + '\n', 'utf8');
console.log('\n✅ Đã patch ' + path + ' (daoduc ky-data)');
