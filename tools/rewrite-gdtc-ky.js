// V6.1.x — Rewrite GDTC ky-data.
const fs = require('fs');
const path = 'engine/data/nhanxet-ky.json';
const GD = require('./data/gdtc-v61.js');

let total = 0, ok = 0, bad = [];
for (const k of Object.keys(GD)) {
    for (const tier of Object.keys(GD[k])) {
        for (const s of GD[k][tier]) {
            total++;
            const L = s.length;
            if (L >= 90 && L <= 110) ok++;
...
const subj = ky.subjects.gdtc;
if (!subj) { console.log('⛔ Không tìm thấy subjects.gdtc'); process.exit(1); }
for (const k of Object.keys(GD)) {
    subj[k] = {};
    for (const tier of Object.keys(GD[k])) subj[k][tier] = GD[k][tier];
}
ky.meta.note_v61x_gdtc = 'V6.1.x (2026-05-23): rewrite GDTC ky-data — 3 kỳ × 4 tier, dạng học bạ TT27, band 90-110.';
fs.writeFileSync(path, JSON.stringify(ky, null, 2) + '\n', 'utf8');
console.log('\n✅ Đã patch ' + path + ' (gdtc ky-data)');
