// V6.1.x — Rewrite Lịch sử & Địa lí ky-data.
const fs = require('fs');
const path = 'engine/data/nhanxet-ky.json';
const LSDL = require('./data/lichsudia-v61.js');

let total = 0, ok = 0, bad = [];
for (const k of Object.keys(LSDL)) {
    for (const tier of Object.keys(LSDL[k])) {
        for (const s of LSDL[k][tier]) {
            total++;
            const L = s.length;
            if (L >= 90 && L <= 110) ok++;
...
const subj = ky.subjects.lichsudia;
if (!subj) { console.log('⛔ Không tìm thấy subjects.lichsudia'); process.exit(1); }
for (const k of Object.keys(LSDL)) {
    subj[k] = {};
    for (const tier of Object.keys(LSDL[k])) subj[k][tier] = LSDL[k][tier];
}
ky.meta.note_v61x_lichsudia = 'V6.1.x (2026-05-23): rewrite Lịch sử & Địa lí ky-data — 3 kỳ × 4 tier, dạng học bạ TT27, band 90-110.';
fs.writeFileSync(path, JSON.stringify(ky, null, 2) + '\n', 'utf8');
console.log('\n✅ Đã patch ' + path + ' (lichsudia ky-data)');
