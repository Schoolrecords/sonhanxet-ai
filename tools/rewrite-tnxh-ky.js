// V6.1.x — Rewrite TNXH ky-data.
const fs = require('fs');
const path = 'engine/data/nhanxet-ky.json';
const TX = require('./data/tnxh-v61.js');

let total = 0, ok = 0, bad = [];
for (const k of Object.keys(TX)) {
    for (const tier of Object.keys(TX[k])) {
        for (const s of TX[k][tier]) {
            total++;
            const L = s.length;
            if (L >= 90 && L <= 110) ok++;
...
for (const k of Object.keys(TX)) {
    for (const tier of Object.keys(TX[k])) {
        for (const s of TX[k][tier]) {
            // TNXH chỉ lớp 1-3 — test gradeLevel=3 (chặt nhất trong dải này)
            const v = e.validateComment(s, {tier, kyCode: k, gradeLevel: 3});
...
const subj = ky.subjects.tnxh;
if (!subj) { console.log('⛔ Không tìm thấy subjects.tnxh'); process.exit(1); }
for (const k of Object.keys(TX)) {
    subj[k] = {};
    for (const tier of Object.keys(TX[k])) subj[k][tier] = TX[k][tier];
}
ky.meta.note_v61x_tnxh = 'V6.1.x (2026-05-23): rewrite TNXH ky-data — 3 kỳ × 4 tier, dạng học bạ TT27, band 90-110.';
fs.writeFileSync(path, JSON.stringify(ky, null, 2) + '\n', 'utf8');
console.log('\n✅ Đã patch ' + path + ' (tnxh ky-data)');
