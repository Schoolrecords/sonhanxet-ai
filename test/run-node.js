...
(async () => {
    ...
    const expectedFields = [
        ...
        'ngon_ngu', 'tinh_toan', 'khoa_hoc', 'tham_mi', 'the_chat',
        'yeu_nuoc', 'nhan_ai', 'cham_chi', 'trung_thuc', 'trach_nhiem'
    ];
    log('có đủ 13 trường', expectedFields.every(k => sg[k] !== undefined),
    ...
    log('HS rỗng: tất cả 13 trường grade=ht (default Đ)',
    ...
    log('fieldsBySection: 5 NL đặc thù', grouped.nang_luc_dac_thu.length === 5);
    ...
    log('autoSuggest HS không có cache: found=false nhưng suggestions vẫn render',
        nf.found === false && Object.keys(nf.suggestions).length === 13);
    log('autoSuggest HS không có: tất cả default Đ',
        Object.values(nf.suggestions).every(f => f.grade === 'ht'));

    // ========== GROUP 9: engine.sinhNLPCDayDu xuất 16 keys ==========
    ...
    const danhGia = {
        ...
        nang_luc_dac_thu: { ngon_ngu: 'tot', tinh_toan: 'tot', khoa_hoc: 'tot', tham_mi: 'tot', the_chat: 'ht' },
        pham_chat: { yeu_nuoc: 'tot', nhan_ai: 'tot', cham_chi: 'tot', trung_thuc: 'tot', trach_nhiem: 'tot' }
    };
    const r = engine.sinhNLPCDayDu({ hoVaTen: 'Test HS' }, danhGia);
    log('engine: NL chung có 4 keys', Object.keys(r.nang_luc_chung).length === 4);
    log('engine: NL đặc thù có 6 keys', Object.keys(r.nang_luc_dac_thu).length === 6);
    log('engine: PC có 6 keys', Object.keys(r.pham_chat).length === 6);
    log('engine: tổng 16 nhận xét',
        Object.keys(r.nang_luc_chung).length + Object.keys(r.nang_luc_dac_thu).length + Object.keys(r.pham_chat).length === 16);

    // Verify mỗi nhận xét là string non-empty
    let allTextValid = true;
    for (const sec of Object.values(r)) {
    ...
    log('engine: tất cả 16 nhận xét là text non-empty', allTextValid);
